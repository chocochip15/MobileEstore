...
<html>
 ...
 <body>
  ...
  <div class="container">
    ...
    <form:form method="post"  modelAttribute="customerDetails"  commandName="customerDetails">
   <table>
   <tr>
   ...
   <c:forEach items="${flowRequestContext.messageContext.getMessagesBySource('custName')}" var="err">
   <div>
    <span>${err.text}</span>
    </div>
   </c:forEach>
   ...
   <!-- to display validate message -->
   <c:forEach items="${flowRequestContext.messageContext.getMessagesBySource('permanent_address')}" var="err">
   <div>
    <span>${err.text}</span>
    </div>
   </c:forEach>
   ...
   <c:forEach items="${flowRequestContext.messageContext.getMessagesBySource('phone_no1')}" var="err">
   <div>
    <span>${err.text}</span>
    </div>
   </c:forEach>
   ...
   <td><form:input path="phone_no2"/></td>
    ...
    <!-- to display validate message -->
   <c:forEach items="${flowRequestContext.messageContext.getMessagesBySource('phone_no2')}" var="err">
   <div>
    <span>${err.text}</span>
    </div>
   </c:forEach>
